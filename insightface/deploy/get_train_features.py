...
parser.add_argument('--train_images_path', default='/wdata/faces_cuts_train', type=str, help='path to images`')

args = parser.parse_args()
model = face_model.FaceModel(args)

train_images_path = args.train_images_path
save_path = '/wdata/mxnet_train_features.pkl'
...
for _file in tqdm(all_files[:]):
    # print(_file)
    person_id = _file.split('/')[-1].split('.')[0].split('_')[0]
